...
import { View, StatusBar,Text,ActivityIndicator,FlatList ,Platform,SafeAreaView,StyleSheet,Button,TouchableOpacity,Image} from 'react-native';
...
const DetailedSales =({navigation,route}) =>{
  ...
  const Item = ({ item, index }) => {
    ...
    return (
      <View style={{ flexDirection: "row", backgroundColor: "#fff", justifyContent: "center", alignItems: "center", padding: 5, margin: 2 }}>
          <View style={{ flexDirection: "row", flex: 2, flexWrap: "wrap" ,padding:2}}>
          <View style={{ flexDirection: "row" }}>
            <Text style={styles.boxText}>{index}.</Text>
            <View>
            <Text style={[styles.boxText, { textAlign: "left",flexShrink: 1,flex:2,padding:6}]}>{item.customer_name||"CASH"}</Text>
   
            <Text style={[styles.boxText, { textAlign: "left" ,color:"grey",fontSize:12,fontStyle:"italic",flex:2,flexWrap: "wrap"}]}>(User:{(item.user_name)})</Text>
        ...
        <Text style={[styles.boxText, { textAlign: "left" }]}>Bill No: {item.billno}</Text>
        <Text style={[styles.boxText, { textAlign: "left" ,fontSize:12,color:"grey"}]}>Time:{tConvert(item.sale_time)}</Text>
            <Text style={[styles.boxText, { textAlign: "left" ,color:"grey",fontSize:12,fontStyle:"italic"}]}>({(item.billtype)})</Text>
            

</View>
<View style={[styles.headerText,{flex:1}]}>

          <Text style={[styles.boxText, { color: "green", fontWeight: "bold", alignSelf: "flex-end" ,fontSize:12}]}>{rounder(item.amount,decimal)}</Text>
        </View>
...
<Header name ="Add Shop" screenName="Bill Wise Sales Report"/>
                </View>
  ...
  {/* <Text style={{color:"red",fontSize:14,fontWeight:"bold",marginLeft:"2%",textAlign:"right"}}> Total Amount :    {rounder(getTotal(data,"amount"),decimal)}</Text> */}
