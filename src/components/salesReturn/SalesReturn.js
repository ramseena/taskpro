...
console.log("enoute",route.params.data)
  ...
  const Item = ({ item, index }) => {
    ...
    return (
      ...
      <View style={{ flexDirection: "row" ,flex:1.8}}>
        ...
        <View style={{}}>
        <Text  numberOfLines={2} style={[styles.boxText, { textAlign: "left"}]}>{item.customer_name||"CASH"}</Text>

        <Text style={[styles.boxText, { textAlign: "left" ,color:"grey",fontSize:12,fontStyle:"italic"}]}>(User:{(item.user_name)})</Text>
        <Text style={[styles.boxText, { textAlign: "left",color:"blue" }]}>PayMode: {item.type}</Text>
        </View>
      </View>

    </View>
    <View style={[styles.headerText,{flex:1,marginLeft:"5%",marginBottom:"3%"}]}>
    
    <Text style={[styles.boxText, { textAlign: "left" }]}>E.No: {item.invno}</Text>
    <Text style={[styles.boxText, { textAlign: "left" }]}>Time:{tConvert(item.return_time)}</Text>
...
<Header name ="Add Shop" screenName="Sales Return Report"/>
                </View>
